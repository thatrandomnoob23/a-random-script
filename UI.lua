...
function module:givegui(player)
	...
	if not player:FindFirstChild("PlayerGui"):FindFirstChild("exec") then
		...
		print("server init")
		game:GetService("ReplicatedStorage"):FindFirstChild("initclient"):FireClient(player, exec, execute2, main, title, buttons, execute, shadow, clear, scriptlist, settingz, exec_2, scripteditor, scriptbox, otherscripts, TextLabel, settings_2, TextLabel_2, require(clientscripts))
		print("client init")
	end
end
